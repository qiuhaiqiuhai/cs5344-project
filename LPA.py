...
################################################################################
# LPA object for holding the graph data structure and running the LPA
# algorithm
################################################################################
class LPA():
    ...
    def __init__(self, filename, num_iterations, threshold, outfile):
        ...
        init_label = {1:1,2:2,3:3}
        # for key, label in init_label.items():
        #     self.graph.node_list[key].label = label
    ...
    def run(self):
        ...
        for i in range(self.num_iterations):
            ...
            self.post_process()
            self.graph.print_communities(self.outfile+'_iter_'+str(i))
        ...
        for random_node in random_nodes:
            ...
            for neighbor in neighbors:
                ...
                if not messages.has_key(message):
                    ...
                    messages[message] += 1
                if messages[message] > most_popular_message_count:
                    most_popular_message_count = messages[message]
                    most_popular_message = [message]
                elif messages[message] == most_popular_message_count:
                    most_popular_message.append(message)

            # Write the most common message received to listener memory
            if not most_popular_message == -1:
                listener.label = random.choice(most_popular_message)
    ...
    def post_process(self):
        ...
        for node in self.graph.node_list.values():
            ...
            self.graph.community_list[key].add(node.name)
...
def main():
    ...
    slpa = LPA(in_file, num_iterations, threshold, out_file)
    slpa.run()
